...
class FeatureEngineer:
    ...
    def add_technical_indicators(
        ...
        stoch = StochasticOscillator(
            high=df["high"], low=df["low"], close=df[price_col]
        )
        ...
        df["obv"] = OnBalanceVolumeIndicator(
            close=df[price_col], volume=df["volume"]
        ).on_balance_volume()
    ...
    def add_sentiment_features(
        self, df: pd.DataFrame, news_df: pd.DataFrame, ticker: str
    ) -> pd.DataFrame:
        """
        ...
        for window in [3, 7, 14]:
            df[f"sentiment_mean_{window}d"] = (
                df["sentiment_mean"].rolling(window).mean()
            )

        return df

    def add_macro_features(
        self, df: pd.DataFrame, macro_data: Dict[str, pd.Series]
    ) -> pd.DataFrame:
        """
...
class TargetGenerator:
    ...
    def volatility_labels(
        self, df: pd.DataFrame, window: int = 20, price_col: str = "close"
    ) -> pd.DataFrame:
        """
