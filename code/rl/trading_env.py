...
class TradingEnv(gym.Env):
    ...
    def __init__(
        ...
        self.data[self.feature_cols] = (
            data[self.feature_cols] - self.feature_mean
        ) / self.feature_std
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, bool, Dict]:
        ...
        truncated = (
            self.total_value <= self.initial_balance * 0.5
        )  # Stop if 50% drawdown
    ...
    def _calculate_reward(self, pnl: float) -> float:
        ...
        if len(self.portfolio_values) > 10:
            returns = (
                np.diff(self.portfolio_values[-10:]) / self.portfolio_values[-10:-1]
            )
    ...
    def _get_observation(self) -> np.ndarray:
        ...
        if len(feature_data) < self.lookback_window:
            padding = np.zeros(
                (self.lookback_window - len(feature_data), self.n_features)
            )
        ...
        portfolio_state = np.array(
            [
                (
                    position_value / self.total_value if self.total_value > 0 else 0
                ),  # Position ratio
                (
                    self.balance / self.total_value if self.total_value > 0 else 0
                ),  # Cash ratio
    ...
    def get_portfolio_stats(self) -> Dict[str, float]:
        ...
        if len(self.trades) > 0:
            profitable_trades = sum(
                1 for t in self.trades if t["action"] == "SELL" and self.position > 0
            )
            win_rate = (
                profitable_trades / len(self.trades) if len(self.trades) > 0 else 0
            )
        else:
            win_rate = 0.0
