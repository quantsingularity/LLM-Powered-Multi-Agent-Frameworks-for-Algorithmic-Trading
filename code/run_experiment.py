...
from data.market_data_loader import MarketDataLoader, SyntheticMarketGenerator, SyntheticNewsGenerator
...
from backtest.backtester import Backtester, BacktestConfig, calculate_bootstrap_confidence_interval
...
class ExperimentRunner:
    ...
    def load_data(self) -> Dict[str, Any]:
        ...
        feature_data = engineer.create_feature_matrix(
            ohlcv_data,
            news_df,
            macro_data if macro_data else None
        )
    ...
    def train_rl_baseline(self, feature_data: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
        ...
        for ticker, df in feature_data.items():
            ...
            train_stats = trainer.train(
                total_timesteps=self.config['rl']['timesteps']
            )
    ...
    def train_hybrid_agent(self, feature_data: Dict[str, pd.DataFrame]) -> Dict[str, Any]:
    ...
    def run_backtests(
        ...
        for ticker, df in feature_data.items():
            ...
            signal_idx = df.index[env.lookback_window:env.lookback_window+len(signals)]
            ...
            logger.info(f"  {ticker} - Total return: {backtest_result['metrics']['total_return']:.2%}")
            logger.info(f"  {ticker} - Sharpe ratio: {backtest_result['metrics']['sharpe_ratio']:.2f}")
    ...
    def _create_architecture_diagram(self):
        ...
        try:
            ...
            dot.node('A', 'Analyst Agent\\n(LLM)', shape='box', style='filled', fillcolor='lightblue')
            dot.node('Dec', 'Decision Agent\\n(LLM)', shape='box', style='filled', fillcolor='lightblue')
            dot.node('R', 'Risk Agent\\n(LLM)', shape='box', style='filled', fillcolor='orange')
            dot.node('E', 'Execution Agent\\n(LLM)', shape='box', style='filled', fillcolor='lightgreen')
            dot.node('Exp', 'Explainability Agent\\n(LLM)', shape='box', style='filled', fillcolor='yellow')
            dot.node('RL', 'RL Policy\\n(PPO/DQN)', shape='box', style='filled', fillcolor='pink')
    ...
    def _plot_equity_curves(self, backtest_results: Dict[str, Any]):
        ...
        fig, axes = plt.subplots(len(backtest_results), 1, figsize=(12, 4*len(backtest_results)))
        ...
        for idx, (ticker, result) in enumerate(backtest_results.items()):
            ...
            # Normalize to 100
            df['portfolio_pct'] = df['portfolio_value'] / df['portfolio_value'].iloc[0] * 100
            df['bh_pct'] = df['price'] / df['price'].iloc[0] * 100
            
            ax.plot(df.index, df['portfolio_pct'], label='Strategy', linewidth=2)
            ax.plot(df.index, df['bh_pct'], label='Buy & Hold', linestyle='--', linewidth=2)
        ...
        plt.tight_layout()
        plt.savefig(f'{self.figures_dir}/equity_curves.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _plot_drawdown_analysis(self, backtest_results: Dict[str, Any]):
        ...
        plt.tight_layout()
        plt.savefig(f'{self.figures_dir}/drawdown_analysis.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _create_sequence_diagram(self):
        ...
        try:
            ...
            dot.render(f'{self.figures_dir}/sequence_diagram', format='png', cleanup=True)
    ...
    def save_results(self, results: Dict[str, Any]):
        ...
        if 'comparison' in results:
            results['comparison'].to_csv(f'{self.results_dir}/metrics/strategy_comparison.csv')
    ...
    def run(self):
        ...
        # Train agents
        rl_baseline = self.train_rl_baseline(data['features'])
...
if __name__ == "__main__":
    ...
    config = {
        ...
        'rl': {
            'algorithm': 'PPO',
            'timesteps': 5000  # Reduced for quick run
        }
    }
