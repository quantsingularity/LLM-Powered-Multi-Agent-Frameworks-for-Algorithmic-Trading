        ...
        for t in range(1, n_periods):
            ...
            if abs(shares_to_trade) > 1e-6:
                ...
                if shares_to_trade > 0:  # Buy
                    ...
                    cash[t] -= shares_to_trade * exec_price - cost  # Note: shares_to_trade is negative
                    position[t] += shares_to_trade
...
def calculate_bootstrap_confidence_interval(
    ...
    sharpe_strategy = np.mean(strategy_returns) / np.std(strategy_returns) if np.std(strategy_returns) > 0 else 0
    sharpe_baseline = np.mean(baseline_returns) / np.std(baseline_returns) if np.std(baseline_returns) > 0 else 0
    ...
    for _ in range(n_bootstrap):
        ...
        sharpe_s = np.mean(strategy_sample) / np.std(strategy_sample) if np.std(strategy_sample) > 0 else 0
        sharpe_b = np.mean(baseline_sample) / np.std(baseline_sample) if np.std(baseline_sample) > 0 else 0
